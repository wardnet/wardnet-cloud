...
//!
//! Production sends the one-time enrollment code via Resend ([`ResendEmailSender`]);
...
/// Sends the enrollment one-time code to a tenant's email.
#[async_trait]
pub trait EmailSender: Send + Sync {
    /// Send the one-time enrollment `code` to `to`.
    ///
    /// # Errors
...
impl EmailSender for ResendEmailSender {
    async fn send_enrollment_code(&self, to: &str, code: &str) -> anyhow::Result<()> {
        let body = ResendEmail {
            from: &self.from,
            to: [to],
            subject: "Your wardnet enrollment code",
            text: format!(
...
impl EmailSender for NoopEmailSender {
    async fn send_enrollment_code(&self, to: &str, code: &str) -> anyhow::Result<()> {
        tracing::info!(to, code, "dev email sender: would send enrollment code");
        Ok(())
    }
