...
#[tokio::test]
async fn find_a_record_returns_first_id_when_present() {
    let server = MockServer::start().await;

    Mock::given(method("GET"))
        .and(path(format!("/zones/{ZONE}/dns_records")))
        .and(query_param("type", "A"))
        .and(query_param("name", "myhost.example.com"))
        .respond_with(ResponseTemplate::new(200).set_body_json(json!({
            "success": true,
            "errors": [],
            "result": [ { "id": "found-a-record" }, { "id": "ignored-dup" } ]
        })))
        .expect(1)
        .mount(&server)
        .await;
    ...
    let id = provider(&server)
        .find_a_record("myhost.example.com")
        .await
        .unwrap();
    assert_eq!(id.as_deref(), Some("found-a-record"));
    server.verify().await;
}

#[tokio::test]
async fn find_a_record_returns_none_on_empty_result() {
    let server = MockServer::start().await;
    ...
    let id = provider(&server)
        .find_a_record("absent.example.com")
        .await
        .unwrap();
    assert_eq!(id, None);
    server.verify().await;
...
#[tokio::test]
async fn find_a_record_errors_when_success_is_false() {
    let server = MockServer::start().await;
    ...
    let result = provider(&server).find_a_record("x.example.com").await;
    assert!(result.is_err());
    assert!(result.unwrap_err().to_string().contains("Invalid zone"));
}
