...
//! orphan rule permits `impl From<LocalError> for ApiError` in the crate owning the
//! local error enum, keeping the domain service HTTP-agnostic. (The Tenants mapping
//! moved to the `wardnet-tenants` crate with `TenantsError` in WS-B.)
