...
/// protocol v1); the listener consumes that header (see [`read_required`] /
/// [`read_optional`]) and injects the **real client address** as `ConnectInfo`,
/// so `addr` here is already the true client and the `X-Forwarded-For` branch
/// below is inert (the peer is never loopback). Threading the real IP this way is
/// load-bearing: the registration rate limit and the IP-bound proof-of-work both
/// depend on it.
///
