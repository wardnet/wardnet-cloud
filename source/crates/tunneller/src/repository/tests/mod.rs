...
//! from `source/`). The pool helper lives here under `cfg(test)`; `sqlx::migrate!`
//! resolves its path relative to this crate.
...
///
/// Requires a `PostgreSQL` server reachable at `TUNNELLER_TEST_DATABASE_URL`
...
async fn fresh_database() -> sqlx::PgPool {
    let base_url = std::env::var("TUNNELLER_TEST_DATABASE_URL")
    ...
    assert!(
        ...
        "TUNNELLER_TEST_DATABASE_URL must be a bare server URL without a /database path, got: {base_url}"
    );
    ...
    // `CREATE DATABASE` is DDL: the identifier cannot be bind-parameterised, so this
    // inline `format!` is the deliberate, test-only exception to the "query strings
    // are `const &str`" convention. The name is a fresh UUID-derived identifier.
