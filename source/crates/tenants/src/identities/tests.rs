//! Unit tests for the Identities aggregate: argon2 round-trip, the two-gate
//! verified-email resolver, and the password / session flows over the shared
//! [`MockStore`].
...
use crate::repository::tenant::Tenant;
use crate::test_helpers::{build_harness, jwt_keypair_pem};
...
async fn password_signup_rejects_bad_code() {
    ...
    let err = h
        ...
        .await
        .unwrap_err();
    assert!(matches!(err, crate::error::IdentitiesError::BadCode(_)));
}
...
async fn password_signup_rejects_weak_password() {
    ...
    let err = h
        ...
        .await
        .unwrap_err();
    assert!(matches!(err, crate::error::IdentitiesError::BadRequest(_)));
}
...
async fn second_password_signup_for_same_email_conflicts() {
    ...
    let err = h
        ...
        .await
        .unwrap_err();
    assert!(matches!(err, crate::error::IdentitiesError::Conflict(_)));
}
