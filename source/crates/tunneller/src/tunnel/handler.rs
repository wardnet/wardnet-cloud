...
use crate::tunnel::registry::{Registration, TunnelRegistry, TunnelStream};
...
/// Time to wait for a Pi READY response before closing the pending stream.
const PENDING_TIMEOUT: Duration = Duration::from_secs(10);
/// Interval at which the node sends a WS-level Ping to detect dead tunnels.
const KEEPALIVE_INTERVAL: Duration = Duration::from_secs(30);
/// Close the tunnel if no message is received from the Pi within this window.
const IDLE_TIMEOUT: Duration = Duration::from_secs(90);

/// Run the WebSocket tunnel loop for one Pi connection (keyed on `slug`).
///
/// Loops until the WebSocket closes, the Pi disconnects, or the tunnel is
/// **aborted** ([`Registration::abort`] fires — a decommission via the reconcile
/// reaper, or a reconnect that displaced this registration). On a clean exit it
/// unregisters (only if it still owns the slug) and deletes its `tunnel_routes`
/// row; on an abort it leaves route deletion to whoever aborted it.
pub async fn run(
    ws: WebSocket,
    slug: String,
    ...
    // Outgoing WebSocket frames (node → Pi).
    let (ws_out_tx, ws_out_rx) = mpsc::channel::<Vec<u8>>(256);
    // stream reader → main loop: (conn_id, data); empty Bytes signals EOF.
    ...
    // conn_id → sender to the stream writer task
    let mut active: HashMap<u32, mpsc::Sender<Bytes>> = HashMap::new();
    // conn_id → stream waiting for READY
    let mut pending: HashMap<u32, Box<dyn TunnelStream>> = HashMap::new();
    ...
    loop {
        tokio::select! {
            // The tunnel was aborted (decommission or displaced by a reconnect).
            () = abort.cancelled() => {
                tracing::info!(slug = %slug, "tunnel aborted, tearing down");
                break;
            }
            ...
            // New inbound connection from the SNI demuxer or a peer node's forward
            req = forward_rx.recv() => {
                ...
                if pending.remove(&conn_id).is_some() {
                ...
                if pending.len() >= MAX_PENDING {
                    ...
                    // req.stream is dropped here, closing the connection.
                    continue;
                }
                ...
                // If the send fails, the ws task has already closed; the loop will
                // exit on the next from_pi_rx or forward_rx recv.
            }

            // Data or EOF from an active stream
    ...
    // Drop the registration only if we still own it (a reconnect would have bumped
    // the generation). On an abort we leave route deletion to the aborter (the
    // reconcile reaper deletes the row before aborting; a reconnect's handler owns
    // the freshly re-upserted row) — so only a clean, still-owned exit deletes it.
    let still_owned = registry.unregister(&slug, generation);
    if still_owned
        && !abort.is_cancelled()
        && let Err(e) = routes.delete(&slug, &node_addr).await
    {
        tracing::warn!(slug = %slug, error = %e, "failed to delete tunnel_routes row on disconnect");
    }
}

/// Drives a `WebSocket` to completion, routing frames via channels.
///
/// Sends a WS-level Ping every [`KEEPALIVE_INTERVAL`] and closes the tunnel if no
/// message arrives within [`IDLE_TIMEOUT`].
async fn drive_ws(
    mut ws: WebSocket,
...
async fn handle_pi_frame(
    ...
    pending: &mut HashMap<u32, Box<dyn TunnelStream>>,
    ...
    match frame_type {
        FRAME_READY => {
            ...
            if let Some(stream) = pending.remove(&conn_id) {
                ...
                let span = tracing::Span::current();
                tokio::spawn(
                    stream_reader(conn_id, read_half, tcp_out_clone).instrument(span.clone()),
                );
                tokio::spawn(stream_writer(write_half, tcp_rx).instrument(span));
            }
        }
        FRAME_DATA => {
            if data.len() > 5
...
async fn stream_writer<W: AsyncWrite + Unpin>(mut writer: W, mut rx: mpsc::Receiver<Bytes>) {
