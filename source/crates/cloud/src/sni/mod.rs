...
//!
//! Behind the transparent L4 proxy the bridge owns two passthrough listeners:
...
//!   tunnel on port 853.
//!
//! The bridge **never terminates** tenant TLS: it is a pure L4 forwarder, so the
//! connection carries the *daemon's* certificate end-to-end. (The control-plane
//! API is served separately, as plain HTTP behind nginx — see `main.rs`.)
//!
...
//! the `ClientHello`, and `copy_bidirectional` (inside the tunnel forward) resumes
//! at it — so the Pi receives raw TLS with no PROXY header, as it expects.
...
/// Run an SNI-demuxing passthrough listener that forwards every tenant connection
/// to its reverse tunnel on `dest_port` (443 for HTTPS, 853 for `DoT`).
///
/// # Errors
...
pub async fn run(
    ...
    loop {
        ...
        tokio::spawn(
            async move {
                let _permit = permit;
                if let Err(e) = route(stream, peer, &suffix, &registry, dest_port).await {
