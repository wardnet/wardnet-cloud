...
- **`crates/tenants`** (bin `wardnet-tenants`, lib `wardnet_tenants`) — the global identity/naming
  ...
  **bootstrap** group (`health`, daemon `enroll` + `token` issue, the unified purpose-bound
  `verification_codes` resource, the Stripe `billing`
  ...
  account plane (`tenants` + the `me` security endpoints — connected login methods, OIDC link,
  sign-out-all — including `deregister`), **plus** a separate **internal mesh-mTLS**
