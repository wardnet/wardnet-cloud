...
async fn handle_pi_frame_short_frame_is_noop() {
    ...
    let mut pending: HashMap<u32, Box<dyn super::TunnelStream>> = HashMap::new();
...
async fn handle_pi_frame_ready_moves_pending_to_active() {
    ...
    let conn_id: u32 = 5;
    pending.insert(conn_id, Box::new(stream));
...
async fn tcp_reader_sends_data_and_eof() {
    ...
    let conn_id: u32 = 99;
    tokio::spawn(stream_reader(conn_id, read_half, out_tx));
...
async fn tcp_writer_sends_bytes_to_socket() {
    ...
    tokio::spawn(stream_writer(write_half, rx));
